  ...
  describe('#Failures in processing', function () {
    ...
    it('Should not process any more records if a record fails and continueProcessingOnError is false', function () {
      ...
      .then(function () {
        ...
        .then(function () {
          expect(onProcessStub.callCount).to.eql(1) // Just the one record was processed
          expect(onProcessStub.getCall(0).args[0].data).to.eql({foo: 1})
          expect(onFailureStub.callCount).to.eql(0)
        })
        .then(function () {
        ...
        .then(function () {
          ...
          expect(onProcessStub.callCount).to.eql(1) // Still just that one record that was processed
          expect(onFailureStub.callCount).to.eql(0)
        })
        .then(getAllRecords)
        .then(function (records) {
          expect(records.length).to.eql(2)
          expect(records[0]).to.have.property('status', 'failed') // First record failed
          expect(records[1]).to.have.property('status', 'received') // Second record was not touched
        })
        .then(function () {
          return Promise.delay(51)
        })
        ...
        .then(function () {
          expect(onProcessStub.callCount).to.eql(3) // The first failed process + the two successes
          expect(onProcessStub.getCall(1).args[0].data).to.eql({foo: 1}) // Make sure in the correct order
          expect(onProcessStub.getCall(2).args[0].data).to.eql({foo: 2})
          expect(onFailureStub.callCount).to.eql(0)
        })
        .then(getAllRecords)
        .then(function (records) {
          ...
          expect(records.length).to.eql(2)
          expect(records[0]).to.have.property('status', 'processed')
          expect(records[1]).to.have.property('status', 'processed')
        })
    })
  })
